...
export default function WorkDetailPage({ params }) {
  ...
  return (
    <article className="px-6 py-8 md:px-10 md:py-12">
      <Link href="/work" className="pill-button mb-4 w-fit">
        Back to Work
      </Link>
      ...
      <h1 className="mt-4 max-w-[840px] text-4xl font-semibold leading-tight md:text-6xl">
        {study.title}
      </h1>
      <p className="mt-3 max-w-[820px] text-sm text-[#4c4c4c] md:text-base">{study.tagline}</p>
      <p className="mt-3 max-w-[820px] text-sm text-[#4c4c4c] md:text-base">{study.summary}</p>

      <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        <div className="rounded-2xl border border-[#d9d7ce] bg-white p-4">
      ...
      <section className="mt-5 rounded-2xl border border-[#d9d7ce] bg-white p-5">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
          {study.outcomes.map((outcome) => (
            <article key={outcome.label} className="rounded-xl border border-[#e2e0d8] bg-[#faf9f6] p-4">
      ...
      <section className="mt-8 rounded-2xl border border-[#d9d7ce] bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          {previousStudy ? (
            <Link href={`/work/${previousStudy.slug}`} className="pill-button px-4 py-2 text-sm">
          ...
          {nextStudy ? (
            <Link href={`/work/${nextStudy.slug}`} className="pill-button px-4 py-2 text-sm">
