"use client";

import Link from "next/link";

export default function WorkDetailError({ reset }) {
  return (
    <section className="px-6 py-8 md:px-10 md:py-12">
      <p className="inline-flex rounded-full border border-[#c8c5ba] bg-white px-3 py-1 text-xs font-semibold">
        Case study unavailable
      </p>
      <h1 className="mt-4 text-3xl font-semibold leading-tight md:text-5xl">
        We couldn&apos;t load this case study details.
      </h1>
      <p className="mt-3 max-w-[680px] text-sm text-[#4c4c4c] md:text-base">
        This can happen if the case study data is missing or temporarily unavailable.
      </p>
      <div className="mt-6 flex flex-wrap items-center gap-2">
        <button type="button" onClick={reset} className="primary-button">
          Retry
        </button>
        <Link href="/work" className="pill-button px-4 py-2 text-sm">
          Back to Work
        </Link>
      </div>
    </section>
  );
}
