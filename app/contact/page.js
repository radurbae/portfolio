...
export default function ContactPage() {
  return (
    <section className="px-6 py-8 md:px-10 md:py-12">
      ...
      <p data-reveal style={{ "--reveal-delay": "120ms" }} className="motion-reveal mt-4 max-w-[760px] text-sm text-[#4c4c4c] md:text-base">
        Share your product goals, timeline, and current challenges. I can support strategy, design,
        and implementation end-to-end.
      </p>

      <div data-reveal style={{ "--reveal-delay": "160ms" }} className="motion-reveal mt-7 grid grid-cols-1 gap-4 md:grid-cols-2">
        <article className="motion-card rounded-2xl border border-[#d9d7ce] bg-white p-5">
          <h2 className="text-xl font-semibold">Email</h2>
          <p className="mt-2 text-sm text-[#4c4c4c]">Best for project scope and requirements.</p>
          <a href={`mailto:${CONTACT_EMAIL}`} className="primary-button motion-press mt-4">
            {CONTACT_EMAIL}
          </a>
        </article>
        ...
        <article className="motion-card rounded-2xl border border-[#d9d7ce] bg-white p-5">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="primary-button motion-press mt-4"
          >
            Book Intro Call
          </a>
        </article>
      ...
      <article data-reveal className="motion-reveal motion-card mt-4 rounded-2xl border border-[#d9d7ce] bg-white p-5">
      ...
      <article data-reveal className="motion-reveal motion-card mt-4 rounded-2xl border border-[#d9d7ce] bg-white p-5">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <a href={`mailto:${CONTACT_EMAIL}`} className="primary-button motion-press">
            Email
          </a>
          <a href={WHATSAPP_URL} target="_blank" rel="noreferrer" className="pill-button motion-press px-4 py-2 text-sm">
            WhatsApp
          </a>
          <a href={CALL_URL} target="_blank" rel="noreferrer" className="pill-button motion-press px-4 py-2 text-sm">
            Book Call
          </a>
        </div>
      </article>
